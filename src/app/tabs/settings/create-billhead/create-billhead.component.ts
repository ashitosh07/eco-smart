...
export class CreateBillheadComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  createBillHead() {
    this.dialog
      .open(BillheadCreateUpdateComponent)
      .afterClosed()
      .subscribe((billhead: Billhead) => {
        /**
        ...
        if (billhead) {
           ...
           */
          this.billHeadService
            .createBillHead(billhead)
            .subscribe((billHeadObj: Billhead) => {
    ...
    this.dialog
      ...
      .subscribe((billHead: Billhead) => {
        ...
        if (billHead) {
           ...
           */
          this.billHeadService
            .updateBillHeadById(billHead.id, billHead)
            .subscribe((billHead: Billhead) => {
  ...
  deleteBillHead(billHead) {
    this.dialog
      .open(UserConfirmationPopupComponent)
      .afterClosed()
  ...
  getBillHeads() {
    ...
    this.billHeadService.getBillHeads(this.clientId).subscribe(
      (billHeads: Billhead[]) => {
        ...
        billHeads.forEach((billHead) => {
          billHead.fixedAmountLocal = this.currencyPipe.transform(
            billHead.fixedAmount,
            this.currency.toString(),
            true,
            this.roundFormat
          )
          billHead.discountAmountLocal = this.currencyPipe.transform(
            billHead.discountAmount,
            this.currency.toString(),
            true,
            this.roundFormat
          )
    ...
    this.data$.pipe(filter((data) => !!data)).subscribe((billHeads) => {
